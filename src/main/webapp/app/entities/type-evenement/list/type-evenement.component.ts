...
export class TypeEvenementComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.typeEvenementService
      ...
      .subscribe(
        (res: HttpResponse<ITypeEvenement[]>) => {
  ...
  trackId(index: number, item: ITypeEvenement): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: ITypeEvenement[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/type-evenement'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
